...
## Update — 2026-08-24: leaves and staff now render; root-to-section connectors

The original "leads only" rule — this view's whole reason for existing being to abstract away from individuals — turned out to be too aggressive: it made "Hide leaves" a permanent, invisible no-op here (hidden from the UI entirely, since it had nothing left to control), and staff positions vanished completely regardless of that switch. Both are now fixed:

**Sections view now uses the exact same `hideLeaves` option `layout-engine.js` already defines**, including its existing carve-out that staff positions are never hidden by it (ADR-005) — `SectionsLayout.calculate(data, { hideLeaves })`. With it off (the default), everyone renders; with it on, anyone with no direct reports of their own is hidden, staff excepted. This is the same option, same semantics, same "staff is a distinct visual category, not a normal report" reasoning as the tree view — Sections view no longer has its own separate leaves policy. Consequently the "Hide leaves" switch (`index.html`) is unconditionally visible again — the `x-show="viewMode !== 'sections'"` guard that hid it is gone, since it's no longer a no-op there.

**A person's *effective* group** is a new concept this required: their own `group`, or — failing that — the nearest ancestor's within the same department (`SectionsLayout._buildSection`'s caller, `calculate()`). Without this, a leaf (which essentially never carries its own `group` in practice — only managers tend to) would render "ungrouped," disconnected from their own team's group box, the moment leaves started rendering at all. Walking the `parentId` chain up to (but not past) the department boundary keeps a leaf visually with their manager instead.

**The root section is now connected to every other section by a line**, drawn as the same orthogonal elbow connector style the tree view's own reporting lines use (`buildElbowPath`, moved from `chart-renderer.js` into the shared `js/utils.js` so both renderers use one implementation) — one line per non-root section, from the root section's bottom-center to that section's top-center. New `.section-link` CSS class (styled like `.org-link`, themed via `--link-stroke`) and a matching export color-baking entry in `file-manager.js`. This is drawn *before* the section boxes so a box's own background paints over the line's arriving end rather than the line drawing over a box edge.

The root section was already centered horizontally above the rest by the previous update, so this addendum is purely additive.

## Update — 2026-08-24: section-lead color is an outline, not a fill

The previous update's "fill the section box with its lead's color" turned out to read as too heavy once every leaf/staff row was in place too — it made a section box look like a single large color swatch rather than an accent on an otherwise-neutral card. Reverted to the lighter treatment: **the section-lead color is now only the box's `stroke`** (a colored accent border), and the fill goes back to the same themed neutral card background (`var(--card-bg)`) every other card in the app uses.

This removed the need for `getContrastTextColor()` in `SectionsLayout._buildSection()` entirely — with the fill now a known, theme-controlled neutral instead of arbitrary per-chart data, there's no more contrast problem to solve. `sectionColor` is still computed the same way (the shallowest person's own color), but it's the only per-section color left; `sectionTextColor` and the `textColor` plumbing through `leadRows`/`groupBoxes` are gone. Group borders/titles, lead name/title text, and the lead-color dot's outline all went back to plain CSS-variable-driven colors (`var(--text-strong)`/`var(--text-muted)`/`var(--dept-text)`/`var(--card-stroke)`), the same values they used before the previous update introduced per-section contrast.

`file-manager.js`'s export baking follows the same split: `.section-rect`'s `fill`, `.section-title`, `.group-rect`'s `stroke`, `.group-title`, `.lead-name`, and `.lead-title` are now baked from theme variables at export time, exactly like `.org-card`'s own classes already are — only `.section-rect`'s `stroke` is left alone, since that's still the one color that's per-chart data (the section lead's own color), not a theme default.
