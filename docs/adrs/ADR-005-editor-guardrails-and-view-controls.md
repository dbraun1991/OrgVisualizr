...
- **Preserves zoom (the default — no `options` passed):** the deep `data` `$watch` (any visual-editor field edit, add/delete node, reparent, "Set Color for All Below", the "Cleanup" action, a canvas click that collapses/expands a subtree), the `hideLeaves` `$watch`, the `language-changed` listener, and hand-typing in the JSON tab (`@input="updateFromJson()"`). All of these keep looking at conceptually the *same* chart — losing your place on every keystroke or toggle would be far more disruptive than a body that doesn't reset.
...
This is a policy call, not just plumbing — the split above is the actual decision this update makes. `layout-engine.js`/`sections-layout.js` and the underlying data model are untouched; this is purely `chart-renderer.js` and the render call sites in `app.js`/`file-manager.js`.
