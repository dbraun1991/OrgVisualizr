...
Noted here rather than in an ADR since none of these is a decision that's been made — just known behaviors and candidate features, kept visible for whoever picks this codebase up next. Not scheduled; no code changes exist yet for any of them.
...
**Sections view: section-lead color should be an outline only, not a fill.** Currently (ADR-006's second Update) a section box's `fill` *is* its section lead's color, with everything drawn inside — group borders/titles, row text, the lead-color dot's outline — switching to whichever of black/white contrasts against it (`getContrastTextColor`). The request is to flip that: the lead's color should only appear as the box's `stroke` (an accent border), with the fill back to the neutral themed card background.
  - *Feasibility*: low-to-moderate, a tweak to existing, already-working code rather than new machinery — no new interaction/state model needed, unlike the two above. In `SectionsLayout._buildSection()` (`js/sections-layout.js`), `sectionColor` would stay computed the same way (the shallowest person's color) but drive `stroke` instead of `fill`; `sectionTextColor` would no longer need to be a contrast computation against an arbitrary color, since the fill reverts to a known, theme-controlled neutral — group titles/row text could very likely go back to plain CSS-variable-driven colors (`var(--text-strong)`/`var(--dept-text)`/etc., as they were before that Update), removing the `textColor` plumbing through `leadRows`/`groupBoxes` and simplifying `sections-renderer.js` back toward its original, first-version form. `file-manager.js`'s export-baking exclusion for these classes would need partial reinstating: `.section-rect`'s `fill` (and probably the text classes) go back to being baked from theme variables like `.org-card` already is, while its `stroke` stays inline per-section, unbaked, same reasoning as now.
